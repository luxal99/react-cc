...
if (processAttr.includes("--reset")) {
  fs.unlink(`${cliPath}/${SETTINGS_FILE_NAME}`, () => {});
  console.log("\x1b[36m%s\x1b[0m", `RCC successfully reset`);
} else {
...
function createComponentFolder() {
  try {
    ...
    if (e.code === "EEXIST") {
      console.log("\x1b[31m", `Component ${componentName} already exists!`);
    }
  }
}
...
function createComponentStyle() {
  fs.appendFile(
    `./${processAttr}/${processAttr}.component${
      cliSettings.typeOfProject === "NEXT" ? ".module" : ""
    }.${cliSettings.typeOfStyle.toLowerCase()}`,
    ``,
    () => {
...
function setDefaultSettings() {
  const applicationTypeInput = prompt(
    "Type of project? \n 1.React \n 2.Next \n"
  );
  const styleTypeInput = prompt(
    "Type of style? \n 1.CSS \n 2.SCSS \n 3. SASS \n 4.LESS \n"
  );
...
function rcc() {
  fs.readdir(`${cliPath}`, (err, files) => {
    const hasSettings = files.includes(SETTINGS_FILE_NAME);
    if (!hasSettings) {
      setDefaultSettings();
      createComponentFolder();
    } else {
      cliSettings = JSON.parse(
        fs.readFileSync(`${cliPath}${SETTINGS_FILE_NAME}`, "utf8")
      );
      createComponentFolder();
    }
  });
